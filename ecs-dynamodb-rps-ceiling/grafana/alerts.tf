  ...
  rule {
    ...
    data {
      ...
      model = jsonencode({
        ...
        expr    = <<-PROMQL
          1 - (
            (histogram_count(sum(rate(http_server_request_duration_seconds{job="ecs-dynamodb-rps-ceiling", http_route!~"/healthz", class=~"fast|standard|heavy", http_response_status_code!~"5.."}[14m]))) or vector(0))
            /
            histogram_count(sum(rate(http_server_request_duration_seconds{job="ecs-dynamodb-rps-ceiling", http_route!~"/healthz", class=~"fast|standard|heavy"}[14m])))
          )
        PROMQL
    ...
    data {
      ...
      model = jsonencode({
        ...
        expr    = <<-PROMQL
          1 - (
            (
              (histogram_fraction(0, 0.05, sum(rate(http_server_request_duration_seconds{job="ecs-dynamodb-rps-ceiling", http_route!~"/healthz", class=~"fast|standard|heavy", class="fast", http_response_status_code!~"5.."}[84m]))) * histogram_count(sum(rate(http_server_request_duration_seconds{job="ecs-dynamodb-rps-ceiling", http_route!~"/healthz", class=~"fast|standard|heavy", class="fast", http_response_status_code!~"5.."}[84m]))) or vector(0))
            +
              (histogram_fraction(0, 0.2, sum(rate(http_server_request_duration_seconds{job="ecs-dynamodb-rps-ceiling", http_route!~"/healthz", class=~"fast|standard|heavy", class="standard", http_response_status_code!~"5.."}[84m]))) * histogram_count(sum(rate(http_server_request_duration_seconds{job="ecs-dynamodb-rps-ceiling", http_route!~"/healthz", class=~"fast|standard|heavy", class="standard", http_response_status_code!~"5.."}[84m]))) or vector(0))
            +
              (histogram_fraction(0, 0.8, sum(rate(http_server_request_duration_seconds{job="ecs-dynamodb-rps-ceiling", http_route!~"/healthz", class=~"fast|standard|heavy", class="heavy", http_response_status_code!~"5.."}[84m]))) * histogram_count(sum(rate(http_server_request_duration_seconds{job="ecs-dynamodb-rps-ceiling", http_route!~"/healthz", class=~"fast|standard|heavy", class="heavy", http_response_status_code!~"5.."}[84m]))) or vector(0))
            )
            /
  ...
  rule {
    ...
    data {
      ...
      model = jsonencode({
        ...
        expr    = <<-PROMQL
          1 - (
            (histogram_count(sum(rate(http_server_request_duration_seconds{job="ecs-dynamodb-rps-ceiling", http_route!~"/healthz", class=~"fast|standard|heavy", http_response_status_code!~"5.."}[84m]))) or vector(0))
            /
            histogram_count(sum(rate(http_server_request_duration_seconds{job="ecs-dynamodb-rps-ceiling", http_route!~"/healthz", class=~"fast|standard|heavy"}[84m])))
          )
        PROMQL
