#!/usr/bin/env bash
# Forked from ecs-dynamodb-rps/scripts/upload-k6.sh on 2026-09-21.
...
#     ./scripts/upload-k6.sh --rate <knee>        # all three, constant/stress pinned to the knee
#
...
# derived `preAllocatedVUs`, and the rate_source tag. Verified against k6 v1.4.0 in the sibling
# project (ecs-dynamodb-rps), with that project's rate: an archive made with `-e RATE=<n>` stored
# rate <n>, the preAllocatedVUs derived from it, and tags.rate_source=explicit. No archive of this
# project's profiles has been checked yet. That is what makes a UI-started run reproducible without
# anyone remembering to visit a settings page.
#
...
#   2. the archive's baked-in BASE_URL is not the endpoint that exists now. A destroy and re-apply
#      cycle gives the ALB a NEW hostname, and that touches no .js file, so check 1 cannot see it.
#      The sibling project's 2026-09-10 case: the archive uploaded 2026-09-09T07:26Z carried the ALB alive at the time,
...
check_stored() {
  ...
  # 2. the endpoint frozen into the archive, against the one that exists now. A destroy and
  #    re-apply cycle gives the ALB a NEW hostname and touches no .js file, so check 1 cannot see
  #    it. The sibling project's 2026-09-10 case: an archive from 2026-09-09T07:26Z kept the ALB alive at that moment, the
