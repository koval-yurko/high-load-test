#!/usr/bin/env bash
# deploy-service.sh [--skip-seed] [--skip-build]
#
# Ships this project's service image to an ALREADY-PROVISIONED environment: build → push →
# force a new ECS deployment → wait for health → seed the table.
#
#     ./scripts/deploy-service.sh [--skip-seed] [--skip-build]
#
# It lives in THIS project's scripts/, not in the repo's, because every step below is specific to how
# this project is built and deployed — ECS Fargate, an ECR repo, a DynamoDB table to seed. The next
...
#
# It deliberately does NOT run `terraform apply`. The PreToolUse guard in .claude/hooks matches
# on command text, so an apply hidden inside a script would never reach the approval gate that
# CLAUDE.md requires before anything billable is created. Provision first, by hand:
...
for arg in "$@"; do
  case "$arg" in
    ...
    *) die "unknown flag '$arg' (usage: ./scripts/deploy-service.sh [--skip-seed] [--skip-build])" ;;
  esac
done
...
header "deploy" "$PROJECT" "Build, push, redeploy, seed. Provisioning is NOT part of this script."
...
if [ "$SKIP_SEED" -eq 1 ]; then
  echo; step "Seed"; info "skipped (--skip-seed)"
elif [ -z "$TABLE" ]; then
  echo; step "Seed"; warn "no table_name output — nothing to seed"
else
  echo
  step "Seeding $TABLE"
  [ -d "service/node_modules" ] || { info "installing service deps first"; ( cd "service" && npm ci ); }
  # Seeding is idempotent, and slow at low write capacity: the retry loop absorbs throttling.
  ( cd "service" && TABLE_NAME="$TABLE" d npm run seed )
  ok "seeded"
fi
...
handoff "deploy" \
  "the current service/src/ running in $PROJECT, table seeded" \
