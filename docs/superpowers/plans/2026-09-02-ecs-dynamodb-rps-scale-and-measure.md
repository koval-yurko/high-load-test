# ecs-dynamodb-rps Scale-and-Measure Implementation Plan
...
**Tech Stack:** Terraform ~1.14 with HCP Terraform (**remote execution**, `working-directory = "infra/main"`,
set by the `platform/` stack),
...
`.superpowers/sdd/2026-09-02-ecs-dynamodb-rps-scale-and-measure/`. The map:
...
> **Renamed and re-pathed 2026-09-03 by the restructure plan**
> (`docs/superpowers/plans/2026-09-03-ecs-dynamodb-rps-restructure.md`): the project is now
> `ecs-dynamodb-rps` and its Terraform root is `infra/main`, its Grafana module `infra/grafana`,
> its load profiles `infra/k6/tests/`, its Node code `service/`. Task 1 Step 1's note about
> deleting the `read_capacity` / `write_capacity` **workspace variables is now done**, confirmed
> 2026-09-04 (that plan's Task 7 Step 4: sixteen hand-set workspace variables deleted, 0 left) —
> skip it, and expect the workspace to carry only the shared variable set.

*The environment was destroyed 2026-09-02, so Task 1's `/env up` is a full create (~52 resources, the
2026-09-04 remote plan read 52 to add / 0 to change / 0 to destroy), not an in-place capacity change:
there is no table to change in place and no seeded data to protect. Task 1 Step 2's expected diff and
the "if the plan proposes to replace the table, stop" instruction apply only to a later re-apply on a
live environment.*
...
5. **Terraform does not rebuild the container image.** Any change under `service/src/` needs an explicit
...
**Files:**
- Modify: `ecs-dynamodb-rps/infra/main/dev.tfvars`
...
`service/src/otel.js` is correct — a forged `k6/v1.4.0` User-Agent produces `traffic_source="k6"` — but **k6
...
header in `infra/k6/tests/lib/request.js`; verify with a 30-second run that a `k6` series appears, before
...
```hcl
# 1. delete these two lines from ecs-dynamodb-rps/infra/main/dev.tfvars
read_capacity  = 25
write_capacity = 25
```
...
**Both paragraphs above assume a live environment, and right now there is none.** The environment was
destroyed 2026-09-02, so this first run is a full create — ~52 to add / 0 to change / 0 to destroy,
as the 2026-09-04 remote plan read — with no table to change in place and no seeded data at risk.
The italic note under the Status banner above has the detail; apply the expected-diff and
do-not-replace checks only on a later re-apply.
...
- [ ] **Step 3: STOP — get explicit approval, then `/env up ecs-dynamodb-rps`**
...
- [ ] **Step 1: Redeploy, so any `service/src/` change is actually running**

```bash
CL=$(terraform -chdir=infra/main output -raw cluster_name)
SV=$(terraform -chdir=infra/main output -raw service_name)
...
Harmless and fast if `service/src/` is untouched. Skipping it when `service/src/` *has* changed means measuring the
...
```bash
/loadtest ecs-dynamodb-rps discovery
```
...
| reading | source | what it answers |
|---|---|---|
| `sli_ratio` and `cpu_saturation_ratio`, plus the per-class latency panels | `infra/grafana/queries.json`, queried against Grafana Cloud Prometheus | how long the endpoints took |
| `ReadThrottleEvents` and `WriteThrottleEvents`, peak per-minute of each | **CloudWatch directly** — `aws cloudwatch get-metric-statistics --namespace AWS/DynamoDB --statistics Sum --period 60` over the run window, taking the max datapoint per metric | whether DynamoDB was rejecting requests |

The two throttle series are **no longer in `queries.json` and no longer in Prometheus**: Tier 1.8 of
the restructure trimmed Alloy's DynamoDB block to `SuccessfulRequestLatency`, so the panel, the
`dynamodb_throttles` alert and `/loadtest`'s `throttles` column all read CloudWatch at the source.
CloudWatch publishes these two metrics sparsely — a minute with no throttling produces no datapoint
at all, not a zero — so an empty `Datapoints` list means nothing was throttled. `/loadtest` carries
the exact command.
...
```bash
git commit --allow-empty -m "test(ecs-dynamodb-rps/k6): freeze load profiles after discovery run"
```
...
k6 traffic labels itself: `traffic_source` is derived from the User-Agent in `service/src/otel.js`, and k6
...
```bash
/loadtest ecs-dynamodb-rps constant
```
...
```bash
/loadtest ecs-dynamodb-rps stress
```
...
- [ ] **Step 3: STOP — approval, then `/env up ecs-dynamodb-rps`**
...
```bash
/loadtest ecs-dynamodb-rps constant --compare
/loadtest ecs-dynamodb-rps stress   --compare
```
...
```bash
git add ecs-dynamodb-rps/infra/main/dev.tfvars
git commit -m "perf(ecs-dynamodb-rps/infra/main): enable ecs autoscaling 1->4 on cpu target"
```
...
**Files:**
- Modify: `ecs-dynamodb-rps/slo.yaml`
- Regenerate: `ecs-dynamodb-rps/infra/main/capacity.auto.tfvars`
...
- [ ] **Step 2: Plan, review, STOP for approval, then `/env up ecs-dynamodb-rps`**
...
```bash
git add ecs-dynamodb-rps/slo.yaml ecs-dynamodb-rps/infra/main/capacity.auto.tfvars
git commit -m "perf(ecs-dynamodb-rps): raise provisioned capacity to release the db ceiling"
```
...
**Files:**
- Modify: `ecs-dynamodb-rps/README.md`
- Create/modify: `ecs-dynamodb-rps/results.md`
...
```bash
grep -c '^|' ecs-dynamodb-rps/results.md
awk -F'|' 'NR>2 && NF>3 && ($4 ~ /^ *$/ || $6 ~ /^ *$/ || $7 ~ /^ *$/ || $13 ~ /^ *$/) \
  { print "INCOMPLETE ROW:", $0 }' ecs-dynamodb-rps/results.md
```
...
- [ ] **Step 3: Update the repo README project table** — replace the `ecs-dynamodb-rps` row's
...
```bash
git add ecs-dynamodb-rps/README.md ecs-dynamodb-rps/results.md README.md
git commit -m "docs(ecs-dynamodb-rps): record measured capacity and before/after results"
```
...
- [ ] **Step 2: STOP — approval, then `/env down ecs-dynamodb-rps`**
...
- **The collector** — ECS service, task definition, security group, IAM role + inline policy, and
  log group `/ecs/ecs-dynamodb-rps-collector`.
- **The heartbeat** — Lambda `ecs-dynamodb-rps-heartbeat`, its EventBridge **Scheduler**
  ...
  `/aws/lambda/ecs-dynamodb-rps-heartbeat`. **The schedule keeps firing until deleted**, so
